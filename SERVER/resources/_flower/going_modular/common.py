...
def set_parameters(net, parameters: List[np.ndarray], context_client=None):
    ...
    if context_client:
        ...
        for k, v in dico.items():
            decrypted_value = v.decrypt(secret_key)
            # Ensure decrypted_value is a sequence
            if isinstance(decrypted_value, (list, np.ndarray)):
                state_dict[k] = torch.Tensor(decrypted_value)
            else:
                state_dict[k] = torch.Tensor([decrypted_value])
