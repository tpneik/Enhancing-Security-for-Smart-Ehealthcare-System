...
def get_on_fit_config_fn(epoch=2, lr=0.0001, batch_size=32) -> Callable[[int], Dict[str, str]]:
...
strategy1= flwr.server.strategy.FedTrimmedAvg(
    ...
    evaluate_fn=None if args.he else evaluate2,
    on_fit_config_fn=get_on_fit_config_fn(epoch=args.max_epochs, lr=args.lr, batch_size=args.batch_size),
)
